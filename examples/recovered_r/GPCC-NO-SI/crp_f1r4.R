...
# Original relative path: GPCC-NO-SI\crp_f1r4.R
# Source SHA256: 34F18E48A6E989E6F46DD720042AA8788A21BF4D84BABDD84227ACB82783FACE
#
...
head(crop)
crop_wheat_f1r4 <- crop %>% filter(Crop == 'Wt'|
                                     Crop == 'Wt-alf'|
                                     Crop == 'Wt_double')

head(crop_wheat_f1r4)

crop_wheat_f1r4 <- crop_wheat_f1r4 %>% mutate(New_Crop = case_when(
...
crop_wheat_f1r4 <- crop_wheat_f1r4 %>% filter(!(New_Crop == 'Alfalfa'|
                                                  New_Crop == 'Sb'))
...
crop %>% group_by(Climate,GCM,New_Crop) %>%
  summarise(Yield = mean(Total_yield)) -> crp_f1r4

crop %>% group_by(Climate,GCM,Tillage,New_Crop) %>%
  summarise(Yield = mean(Total_yield)) -> crp_till_f1r4
