...
# Original relative path: GPCC-NO-SI\crp_f2r8.R
# Source SHA256: D19B456251F800A7E2EA4C07FDAED2C92F29D8D55D21F18545D16C29D84B4A84
#
...
head(crop)
crop_wheat_f2r8 <- crop %>% filter(Crop == 'Wt'|
                                     Crop == 'Wt-alf'|
                                     Crop == 'Wt_double')

head(crop_wheat_f2r8)

crop_wheat_f2r8 <- crop_wheat_f2r8 %>% mutate(New_Crop = case_when(
...
crop_wheat_f2r8 <- crop_wheat_f2r8 %>% filter(!(New_Crop == 'Alfalfa'|
                                                  New_Crop == 'Sb'))
#----------------------------------------------
class(crop)
...
crop %>% group_by(Climate,GCM,Tillage,New_Crop) %>%
  summarise(Yield = mean(Total_yield)) -> crp_till_f2r8
