...
# Original relative path: GPCC-SI\Table 5.R
# Source SHA256: E9C972D86B0D651D08DFFF64F6CD4177348FF5B37CEF0FF9F71B1CCB0142D718
#
...
# ============================================================================
data <- read.csv('Table5-GPCC-SI.csv')
data
...
# finally plot runoff 
library(wesanderson)
ann_text <- data.frame(Tillage_system = 'NT',Crop = 'Sg-alf',lab = '(mm)',Climate='Baseline')
...
data %>% arrange(desc(Runoff)) %>% 
  ...
  geom_text(aes(x=Crop,y=reorder(Tillage_system,Runoff),label=Sig_run),stat='identity') +
...
ann_text1 <- data.frame(Tillage_system = 'DT',Crop = 'Ct-alf',Climate='F1R4.5')
data %>% 
  ...
  geom_text(aes(x=factor(Crop,ordered = T,levels = c('Sb','Ct','Sg','Ca','Wt',
                                                     'Wt_double','Ca-alf','Sg-alf',
                                                     'Sb-alf','Ct-alf','Wt-alf')),
                y=factor(Tillage_system,ordered = T,levels = c('NT','DT','CT','RT')),label=Sig_soil),stat='identity') +
